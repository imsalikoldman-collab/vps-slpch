...
import type { PartnerCardInput } from "@/types/partners";
...
export function parsePartnerCardInput(payload: unknown): { ok: true; data: PartnerCardInput } | { ok: false; error: string } {
  if (!isObject(payload)) {
  ...
  const titleHref = payload.titleHref === null || payload.titleHref === undefined ? null : String(payload.titleHref).trim();
